<template>
    <AppLayout>
        <div class="p-8 space-y-6">
                ...
                <div class="bg-white rounded-xl border border-dark-100 p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-semibold text-dark-900 mt-1">{{ departmentsStore.departments.length
                                }}</p>
                        </div>
            ...
            <!-- Grid -->
            <div v-else-if="departmentsStore.departments.length === 0"
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                <div v-for="department in departmentsStore.departments" :key="department.id"
                        ...
                        <div class="flex items-center justify-between mt-4 pt-3 border-t border-dark-100">
                            <div class="flex items-center gap-2">
                                <div class="flex items-center gap-1">
                                    ...
                                    <span class="text-sm text-dark-600">{{ department.agents_count || 0 }}
                                        agent(s)</span>
                                </div>
                            </div>
                            <span
        ...
        <div v-if="modalOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
            ...
            <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4">
                <div class="flex items-center justify-between p-5 border-b border-dark-100">
                    <h2 class="text-lg font-semibold text-dark-900">{{ editingDepartment ? 'Modifier le département' :
                        'Nouveau département' }}</h2>
                ...
                <form @submit.prevent="saveDepartment" class="p-5 space-y-4">
                    <AppInput v-model="form.name" label="Nom du département" required :error="ve.name?.[0]" />

                    <AppInput v-model="form.code" label="Code" required placeholder="ex: RH, IT, MKT"
                        :error="ve.code?.[0]" />

                    <AppInput v-model="form.color" label="Couleur" type="color" :error="ve.color?.[0]" />
                    ...
                    <AppInput v-model="form.description" label="Description" type="textarea" rows="3"
            ...
            <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4">
                ...
                <div class="p-5">
                    <p class="text-dark-700">
                        Êtes-vous sûr de vouloir supprimer le département <strong>{{ departmentToDelete?.name
                            }}</strong> ?
                    </p>
...
// ✅ Correction ici aussi : filtre sécurisé
const activeCount = computed(() => {
    return departmentsStore.departments.filter(d => d && d.is_active === true).length
})
...
function openEditModal(department: Department) {
    ...
    form.value = {
        ...
        is_active: department.is_active ?? true // ✅ Valeur par défaut
    }
    modalOpen.value = true
}
...
async function saveDepartment() {
    ...
    try {
        ...
        closeModal()
        await loadDepartments() // Recharger pour mettre à jour la liste
    } catch (error: any) {
...
async function deleteDepartment() {
    ...
    try {
        ...
        toast.success(`Département "${departmentToDelete.value.name}" supprimé avec succès`)
        closeDeleteModal()
    } catch (error: any) {
