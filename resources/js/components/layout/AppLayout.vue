    ...
    <aside class="w-64 bg-white border-r border-dark-100 flex flex-col">
      ...
      <div class="border-t border-dark-100 p-4">
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white font-semibold text-sm shadow-sm">
            {{ userInitials }}
          </div>
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-dark-900 truncate">{{ authStore.user?.name }}</p>
            <p class="text-xs text-dark-500 truncate capitalize">{{ getUserRoleLabel(authStore.user?.roles?.[0]) }}</p>
          </div>
          <button
            title="Déconnexion"
